...
function SignupModal({ onCloseBtnClick, onSwitchToLogin }) {
  const { signupUser, error: signupError, loading: isLoading } = useSignup()
  ...
  const submitForm = (e) => {
    ...
    const { username, password, password2 } = Object.fromEntries(formData) // convert the FormData object to a JSON object

    signupUser(username, password, password2, onCloseBtnClick)
  }

  return (
    <>
      <div className="z-100 flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-md space-y-8">
          <div>
            <span className="mx-auto flex h-10 w-10">
              <Logo />
            </span>
            <h2 className="mt-6 text-center text-3xl font-extrabold tracking-tight text-white drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]">
              Create new account
            </h2>
          </div>
            ...
            <div className="-space-y-px rounded-md shadow-sm">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="relative block w-full appearance-none rounded-none border border-t-0 border-white/20 bg-black/20 px-3 py-3 text-white placeholder-gray-400 backdrop-blur-md transition-colors focus:z-10 focus:border-primary-500 focus:outline-none focus:ring-primary-500 sm:text-sm"
